# Semantic Category Duel — Stage 2
...
Common storage fields, same32 cases/32 revisions. Constructor common. Create base `{category:'ANIMAL'|'PLANT'|'FOOD'|'TOOL',initial_letter:single lowercase ASCII letter}`; cap256. Response `{word:T(24)}` only when move submitted; word `[a-z]{2,24}`. Base immutable from create. Domain exact `{turn:0..6,score_a:0..3,score_b:0..3,last_letter:single char,used:L(6,T(24)),moves:L(6,{turn:0..5,word:E(24),result:'VALID'|'INVALID_CATEGORY'|'BAD_LINK'|'REPEATED'|'PASS',player:'A'|'B'}),joined:bool}`. Move order chronological, used insertion order unique valid words. Primary playerA, secondary nominated playerB distinct/nonzero. Turn0/2/4 A,1/3/5 B. Scores reset0, last_letter initial. Game phase INVITED until join then TURN. No edit-base method. No parent/revisions to replay a finished game: parent must0 at create.

At play: turn/caller/CAS check first. Freeze word to response and phase FROZEN, accepted_attempts0,last_accepted_at0,outcome empty,result{}. A bad first letter or word already in used is still a submitted move; evaluate deterministically scores0 and advances without LLM (BAD_LINK precedence over REPEATED). Otherwise one consensus result exact `{v:1,label:'IN_CATEGORY'|'OUT_OF_CATEGORY'|'UNKNOWN'}`. Rule common English noun in frozen category, not proper names, abbreviations or invented terms; interpret most common ordinary noun sense, ambiguity UNKNOWN. IN_CATEGORY gives current player+1, appends used and last_letter=word[-1]. OUT_OF_CATEGORY gives0, doesn't change last letter/used. All conclusive moves append move record and increment turn. If turn6, phase DONE and outcome A_WINS/B_WINS/DRAW from scores; otherwise phase TURN, outcome empty but move record persists. UNKNOWN uses UNRESOLVED/EXHAUSTED from shared retry, no turn or score advance. Only current player may `pass_turn` from TURN/FROZEN/UNRESOLVED/EXHAUSTED; scores0, append PASS/empty word and advance. Pass is also allowed from FROZEN after a failed/no-write evaluation so model unavailability cannot deadlock the game. Expected revision serializes a race: whichever of evaluation/pass commits first invalidates the other's expected revision; the frontend still forbids submitting while its known pending hash is unreconciled. `resign` by either player in INVITED/TURN/FROZEN/UNRESOLVED/EXHAUSTED ends DONE with opponent winner; CAS serializes any evaluation race. Current phase/history prove whether a transaction already progressed. One join + six maximum submit/3accepted assessments/pass cycles + create fits32 revisions (2+6*5=32); if last pass fills32 it also ends game. Early resign always occurs at a revision≤31.

| Public write signature | Caller/validation | Mutation/readback |
|---|---|---|
| create_game(nonce:str,opponent:Address,category:str,initial_letter:str)->u256 | any CREATE, distinct opponent, fixed enums | INVITED, domain initialized, base_locked=true |
| join_game(id:u256,expected_revision:u256)->None | nominated B, INVITED | joined=true, TURN, rev+1 |
| play_word(id:u256,word:str,turn:u256,expected_revision:u256)->None | current player, TURN, exact turn, word grammar | FROZEN, exact word, reset attempt counters |
| evaluate_move(id:u256,expected_revision:u256)->None | anyone FROZEN attempts0 | apply deterministic reject or consensus rules |
| retry_move(id:u256,expected_revision:u256)->None | anyone UNRESOLVED, attempts<3,cooldown | shared retries, or conclusive move advance |
| pass_turn(id:u256,turn:u256,expected_revision:u256)->None | current player, TURN/FROZEN/UNRESOLVED/EXHAUSTED | PASS record and next turn or DONE |
| resign_game(id:u256,expected_revision:u256)->None | A/B, allowed phases above | DONE opponent winner, no points fabricated |

Common views give complete game including ≤6 moves; history per version. Record below8192, result≤128. Actor_index lists both players. Nonce/capacity common. Only current_move response is overwritten at next play; historical moves and exact prior responses remain history. Signature rejection, consensus failure/no-write do not consume turns.

Screens: New game opponent/category/letter; Invitation with B Join and both Resign; Arena displays turn/player, last letter, used words, scores,6-row move ledger; current player Word input/Play or Pass; observers Evaluate; unresolved Retry and current player Pass; Finished outcome. Copy `Game score from accepted word-category judgments; no rewards or verified real-world achievement.` Wallet switch finds existing journal and does not autojoin/play. Tests valid ant→tiger, category wrong, wrong first letter, repeated word, wrong turn/player, evaluation/pass CAS race, unknown attempts+pass, no-write counters, six-turn score/draw, resign, maximal32 revisions, every method/view and journal intent.

WRONG:
```python
score += 1  # word submitted, before accepted semantic result
```
CORRECT:
```text
Submission only freezes word. Award point and advance turn atomically only after accepted IN_CATEGORY, or advance zero-point deterministic/conclusive invalid/pass. UNKNOWN and no consensus never award points.
```
Symptom free points/replayed moves; root premature scoring; verify `pytest tests/test_contract.py -k game_turn`; fixed when failed/disagreed evaluation preserves score/turn and repeated expected_revision cannot advance twice.
